...
class Template27 {
  ...
  Resume27ThemeModel resume27themeModel;
  ...
  Template27({required this.resume27themeModel}) {
    normalTextColor = resume27themeModel.normalTextColor ?? PdfColors.black;
    headerDarkColor = resume27themeModel.headerColor ?? PdfColors.grey800;
    // headerLightColor is not in Resume21ThemeModel, using default or derived
    headerLightColor = PdfColors.white;

    nameColor = resume27themeModel.nameColor ?? PdfColors.blueGrey900;
    positionColor = resume27themeModel.positionColor ?? PdfColors.blueGrey900;

    backgroundPath = resume27themeModel.background ?? PdfAssets.back101;
    linkAsset = resume27themeModel.linkIc ?? PdfAssets.link7;

    // Initialize bullet paths
    bulletDarkPath = resume27themeModel.bullet72 ?? PdfAssets.bullet7;
    bulletLightPath = resume27themeModel.bullet72 ?? PdfAssets.bullet7;

    normalFont = resume27themeModel.normalFont ?? Font.times();
    headerFont = resume27themeModel.headerFont ?? Font.helvetica();
    nameFont = resume27themeModel.headerFont ?? Font.helveticaBold();
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template27: Main resume document not found.");
      }
      ...
      if (bulletDarkPath != null && bulletDarkPath!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bulletDarkPath!).then((byteData) {
          bulletDarkImage = MemoryImage(byteData.buffer.asUint8List());
        }).catchError((e) {
          print(
              "Template27: Failed to load bullet dark asset $bulletDarkPath: $e");
        }));
      }

      if (bulletLightPath != null && bulletLightPath!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bulletLightPath!).then((byteData) {
          bulletLightImage = MemoryImage(byteData.buffer.asUint8List());
        }).catchError((e) {
          print(
              "Template27: Failed to load bullet light asset $bulletLightPath: $e");
        }));
      }
      ...
      print("Template27: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template27: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template27: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template27: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template27: Building PDF widgets...");
    print(
        " Template27: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template27: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template27: summaryData=${summaryData != null ? 'exists' : 'null'}");
    print(" Template27: workData.length=${workData.length}");
    print(" Template27: educationData.length=${educationData.length}");
    print(" Template27: sectionData.length=${sectionData.length}");
