...
class Template10 {
  ...
  MemoryImage? imageBorder1;
  MemoryImage? email_41;
  MemoryImage? phone_41;
  MemoryImage? link_41;

  Resume10ThemeModel? resume10theme;
  ...
  Template10({required this.resume10theme}) {
    normalFont = resume10theme?.normalFont ?? Font.helvetica();
    headerFont = resume10theme?.headerFont ?? Font.helveticaBold();
    subHeaderColor =
        resume10theme?.subHeaderColor ?? PdfColor.fromHex("#1E2A5A");
    headerColor = resume10theme?.headerColor ?? PdfColor.fromHex("#1E2A5A");
    nameColor = resume10theme?.nameColor ?? PdfColor.fromHex("#1E2A5A");
    background = resume10theme?.background ?? PdfAssets.rect41;
    normalTextColor =
        resume10theme!.normalTextColor ?? PdfColor.fromHex("#FFFFFF");
    positionColor = resume10theme!.positionColor ?? PdfColor.fromHex("#F0F0F0");
    boxBottomBorderColor =
        resume10theme!.boxBottomBorderColor ?? PdfColor.fromHex("#1E2A5A");
    backgroundBoxColor =
        resume10theme!.backgroundBoxColor ?? PdfColor.fromHex('#8B0000');
    bullet = resume10theme?.bullet;
    imageBorder = resume10theme?.imageBorder1;
    emailIc = resume10theme?.emailIc;
    phoneIc = resume10theme?.phoneIc;
    linkIc = resume10theme?.linkIc;
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      print("Error in Template10: User or Resume ID is null.");
      return false;
    }
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template10: Main resume document not found.");
      }
      ...
      if (imageBorder != null && imageBorder!.isNotEmpty) {
        imageFutures.add(rootBundle.load(imageBorder!).then(
            (data) => imageBorder1 = MemoryImage(data.buffer.asUint8List())));
      }

      if (bullet != null && bullet!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bullet!).then((byteData) {
      ...
      if (emailIc != null && emailIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(emailIc!)
            .then((data) => email_41 = MemoryImage(data.buffer.asUint8List())));
      }
      if (phoneIc != null && phoneIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(phoneIc!)
            .then((data) => phone_41 = MemoryImage(data.buffer.asUint8List())));
      }
      if (linkIc != null && linkIc!.isNotEmpty) {
        ...
        imageFutures.add(rootBundle
            .load(linkIc!)
            .then((data) => link_41 = MemoryImage(data.buffer.asUint8List())));
      }
      ...
      print("Template10: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template10: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template10: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template10: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template10: Building PDF widgets...");
    print(
        " Template10: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template10: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template10: summeryData=${summeryData != null ? 'exists' : 'null'}");
    print(" Template10: workData.length=${workData?.length}");
    print(" Template10: educationData.length=${educationData?.length}");
    print(" Template10: sectionData.length=${sectionData.length}");
                ...
                children: [
                  if (profileImage != null)
                    Container(
                      width: 120,
                      height: 120,
                      padding: imageBorder1 != null
                          ? EdgeInsets.all(8)
                          : EdgeInsets.zero,
                      decoration: imageBorder1 != null
                          ? BoxDecoration(
                              image: DecorationImage(
                                  image: imageBorder1!, fit: BoxFit.cover),
            ...
            Padding(
              ...
              child: _buildRotatedSection(
                ...
                content: Column(
                  ...
                  children: [
                    if (emailIc != null && email_41 != null)
                      _buildContactRow(email_41!, contactData?.email),
                    if (phoneIc != null && phone_41 != null)
                      _buildContactRow(phone_41!, contactData?.phone),
                    if (linkIc != null && link_41 != null)
                      _buildContactRow(link_41!, contactData?.socialMediaUrl1),
                    if (linkIc != null && link_41 != null)
                      _buildContactRow(link_41!, contactData?.socialMediaUrl2),
