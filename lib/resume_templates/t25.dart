...
class Template25 {
  ...
  Template25({required Resume25ThemeModel? resume25themeModel}) {
    subHeaderColor =
        resume25themeModel?.subHeaderColor ?? PdfColor.fromHex("#B87397");
    headerColor =
        resume25themeModel?.headerColor ?? PdfColor.fromHex("#B87397");
    nameColor = resume25themeModel?.nameColor ?? PdfColor.fromHex("#B87397");
    normalTextColor =
        resume25themeModel!.normalTextColor ?? PdfColor.fromHex("#0D490E");
    backgroundColor = resume25themeModel.backgroundColor;
    bullet82 = resume25themeModel.bullet82;
    background = resume25themeModel.background;
    phoneIc = resume25themeModel.phoneIc;
    linkIc = resume25themeModel.linkIc;
    normalFont = resume25themeModel.normalFont ?? Font.helvetica();
    headerFont = resume25themeModel.headerFont ?? Font.helveticaBold();
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      print("Error in Template25: User or Resume ID is null.");
      return false;
    }
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template25: Main resume document not found.");
      }
      ...
      if (bullet82 != null && bullet82!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bullet82!).then((byteData) {
      ...
      print("Template25: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template25: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template25: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template25: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template25: Building PDF widgets...");
    print(
        " Template25: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template25: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template25: summeryData=${summeryData != null ? 'exists' : 'null'}");
    print(" Template25: workData.length=${workData.length}");
    print(" Template25: educationData.length=${educationData.length}");
    print(" Template25: sectionData.length=${sectionData.length}");
      ...
      Container(
        ...
        decoration: BoxDecoration(
          ...
          gradient: LinearGradient(
            colors: [
              PdfColor.fromHex('#8CB9D4'),
              PdfColor.fromHex('#7FB0CF'),
            ],
