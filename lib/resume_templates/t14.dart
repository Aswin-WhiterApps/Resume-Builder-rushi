...
class Template14 {
  ...
  MemoryImage? imageBorder52;
  MemoryImage? email_52;
  MemoryImage? phone_52;
  MemoryImage? link_52;
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      print("Error in Template14: User or Resume ID is null.");
      return false;
    }
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template14: Main resume document not found.");
      }
      ...
      if (emailIc != null && emailIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(emailIc!)
            .then((data) => email_52 = MemoryImage(data.buffer.asUint8List())));
      }
      if (phoneIc != null && phoneIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(phoneIc!)
            .then((data) => phone_52 = MemoryImage(data.buffer.asUint8List())));
      }
      if (linkIc != null && linkIc!.isNotEmpty) {
        ...
        imageFutures.add(rootBundle
            .load(linkIc!)
            .then((data) => link_52 = MemoryImage(data.buffer.asUint8List())));
      }
      ...
      print("Template14: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template14: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template14: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template14: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template14: Building PDF widgets...");
    print(
        " Template14: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template14: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template14: summeryData=${summeryData != null ? 'exists' : 'null'}");
    print(" Template14: workData.length=${workData.length}");
            ...
            if (profileImage != null)
              Container(
                ...
                decoration: imageBorder52 != null
                    ? BoxDecoration(
                        image: DecorationImage(
                            image: imageBorder52!, fit: BoxFit.cover),
