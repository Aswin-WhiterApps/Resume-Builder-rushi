...
class Template18 {
  ...
  MemoryImage? imageBorder62;
  MemoryImage? email_62;
  MemoryImage? phone_62;
  MemoryImage? link_62;
  ...
  Template18(
      {required Resume18ThemeModel? resume18theme}) {
    subHeaderColor = resume18theme?.subHeaderColor ?? PdfColor.fromHex("#B87397");
    headerColor = resume18theme?.headerColor ?? PdfColor.fromHex("#B87397");
    nameColor = resume18theme?.nameColor ?? PdfColor.fromHex("#B87397");
    normalTextColor = resume18theme!.normalTextColor ?? PdfColor.fromHex("#0D490E");
    bullet62 = resume18theme.bullet62;
    background = resume18theme.rect62 ?? PdfAssets.rect62;
    imageBorder = resume18theme.imageBorder62;
    emailIc = resume18theme.emailIc;
    phoneIc = resume18theme.phoneIc;
    linkIc = resume18theme.linkIc;
    normalFont = resume18theme.normalFont ?? Font.helvetica();
    headerFont = resume18theme.headerFont ?? Font.helveticaBold(); 
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      print("Error in Template18: User or Resume ID is null.");
      return false;
    }
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template18: Main resume document not found.");
      }
      ...
      if (imageBorder != null && imageBorder!.isNotEmpty) {
        imageFutures.add(rootBundle.load(imageBorder!).then(
            (data) => imageBorder62 = MemoryImage(data.buffer.asUint8List())));
      }

      if (bullet62 != null && bullet62!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bullet62!).then((byteData) {
      ...
      if (emailIc != null && emailIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(emailIc!)
            .then((data) => email_62 = MemoryImage(data.buffer.asUint8List())));
      }
      if (phoneIc != null && phoneIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(phoneIc!)
            .then((data) => phone_62 = MemoryImage(data.buffer.asUint8List())));
      }
      if (linkIc != null && linkIc!.isNotEmpty) {
        ...
        imageFutures.add(rootBundle
            .load(linkIc!)
            .then((data) => link_62 = MemoryImage(data.buffer.asUint8List())));
      }

      if (background != null && background!.isNotEmpty) {
        imageFutures.add(rootBundle.load(background!).then((data) =>
            imageBorder62 = MemoryImage(data.buffer.asUint8List())));
      }
      ...
      if (introData?.imagePath != null && introData!.imagePath!.isNotEmpty) {
        ...
        if (await imageFile.exists()) {
          imageFutures.add(imageFile
              .readAsBytes()
              .then((bytes) => imageBorder62 = MemoryImage(bytes)));
        }
      }

      await Future.wait(imageFutures);
      print("Template18: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template18: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template18: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template18: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template18: Building PDF widgets...");
    print(
        " Template18: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template18: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template18: summeryData=${summeryData != null ? 'exists' : 'null'}");
    print(" Template18: workData.length=${workData.length}");
    print(" Template18: educationData.length=${educationData.length}");
    print(" Template18: sectionData.length=${sectionData.length}");

     final pdf = Document();

    print(" Template17: All widgets built successfully");
    ...
    pdf.addPage(MultiPage(
        pageTheme: PageTheme(
            ...
            buildBackground: (context) {
              return imageBorder62 != null
                  ? Container(
                      decoration: BoxDecoration(
                          image: DecorationImage(
                              fit: BoxFit.cover, image: imageBorder62!)))
    ...
    try {
      ...
      final file = File("${outputDir.path}/Resume_T18_$timestamp.pdf");
      await file.writeAsBytes(await pdf.save());
      print("Pdf Saved To ${file.path}");
      return file;
    } catch (e) {
      print('Failed to save PDF for Template17: $e');
      return null;
    }
  }
  ...
  SectionModel? projectsSection;
    ...
    rightColumnItems.add(
      Container(
        padding: EdgeInsets.only(top: 60, bottom: 5, right: 40, left: 20),
        child: Column(
          ...
          children: [
            if (imageBorder != null)
              Container(
                ...
                height: 120,
                padding:
                    imageBorder62 != null ? EdgeInsets.all(5) : EdgeInsets.zero,
                decoration: imageBorder62 != null
                    ? BoxDecoration(
                        image: DecorationImage(
                            image: imageBorder62!, fit: BoxFit.cover),
                ...
                child: Container(
                  ...
                  child:
                      ClipOval(child: Image(imageBorder62!, fit: BoxFit.cover)),
                ),
              ),
          ],
        ),
            ...
            Padding(
              ...
              child: _buildRotatedSection(
                ...
                content: Column(
                  ...
                  children: [
                    if (emailIc != null && email_62 != null)
                      _buildContactRow(email_62!, contactData?.email),
                    if (phoneIc != null && phone_62 != null)
                      _buildContactRow(phone_62!, contactData?.phone),
                    if (linkIc != null && link_62 != null)
                      _buildContactRow(link_62!, contactData?.socialMediaUrl1),
                    if (linkIc != null && link_62 != null)
                      _buildContactRow(link_62!, contactData?.socialMediaUrl2),
