...
class Template20 {
  ...
  MemoryImage? email_64;
  MemoryImage? phone_64;
  MemoryImage? link_64;
  ...
  Template20({required Resume20ThemeModel? resume20theme}) {
    subHeaderColor =
        resume20theme?.subHeaderColor ?? PdfColor.fromHex("#B87397");
    headerColor = resume20theme?.headerColor ?? PdfColor.fromHex("#B87397");
    nameColor = resume20theme?.nameColor ?? PdfColor.fromHex("#B87397");
    normalTextColor =
        resume20theme!.normalTextColor ?? PdfColor.fromHex("#0D490E");
    bullet64 = resume20theme.bullet64;
    background = resume20theme.rect64 ?? PdfAssets.rect64;
    imageBorder = resume20theme.imageBorder64;
    emailIc = resume20theme.emailIc;
    phoneIc = resume20theme.phoneIc;
    linkIc = resume20theme.linkIc;
    normalFont = resume20theme.normalFont ?? Font.helvetica();
    headerFont = resume20theme.headerFont ?? Font.helveticaBold();
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      print("Error in Template20: User or Resume ID is null.");
      return false;
    }
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template20: Main resume document not found.");
      }
      ...
      if (imageBorder != null && imageBorder!.isNotEmpty) {
        imageFutures.add(rootBundle.load(imageBorder!).then(
            (data) => imageBorder64 = MemoryImage(data.buffer.asUint8List())));
      }

      if (bullet64 != null && bullet64!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bullet64!).then((byteData) {
      ...
      if (emailIc != null && emailIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(emailIc!)
            .then((data) => email_64 = MemoryImage(data.buffer.asUint8List())));
      }
      if (phoneIc != null && phoneIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(phoneIc!)
            .then((data) => phone_64 = MemoryImage(data.buffer.asUint8List())));
      }
      if (linkIc != null && linkIc!.isNotEmpty) {
        ...
        imageFutures.add(rootBundle
            .load(linkIc!)
            .then((data) => link_64 = MemoryImage(data.buffer.asUint8List())));
      }

      if (background != null && background!.isNotEmpty) {
        imageFutures.add(rootBundle.load(background!).then(
            (data) => imageBorder64 = MemoryImage(data.buffer.asUint8List())));
      }
      ...
      if (introData?.imagePath != null && introData!.imagePath!.isNotEmpty) {
        ...
        if (await imageFile.exists()) {
          imageFutures.add(imageFile
              .readAsBytes()
              .then((bytes) => imageBorder64 = MemoryImage(bytes)));
        }
      }

      await Future.wait(imageFutures);
      print("Template20: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template20: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template20: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template20: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template20: Building PDF widgets...");
    print(
        " Template20: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template20: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template20: summeryData=${summeryData != null ? 'exists' : 'null'}");
    print(" Template20: workData.length=${workData.length}");
    print(" Template20: educationData.length=${educationData.length}");
    print(" Template20: sectionData.length=${sectionData.length}");
    ...
    pdf.addPage(MultiPage(
        pageTheme: PageTheme(
            ...
            buildBackground: (context) {
              return imageBorder64 != null
                  ? Container(
                      decoration: BoxDecoration(
                          image: DecorationImage(
                              fit: BoxFit.cover, image: imageBorder64!)))
          ...
          children: [
            if (imageBorder != null)
              Container(
                ...
                height: 120,
                padding:
                    imageBorder64 != null ? EdgeInsets.all(5) : EdgeInsets.zero,
      ...
      Padding(
        ...
        child: _buildRotatedSection(
          ...
          content: Column(
            ...
            children: [
              if (emailIc != null && email_64 != null)
                _buildContactRow(email_64!, contactData?.email),
              if (phoneIc != null && phone_64 != null)
                _buildContactRow(phone_64!, contactData?.phone),
              if (linkIc != null && link_64 != null)
                _buildContactRow(link_64!, contactData?.socialMediaUrl1),
              if (linkIc != null && link_64 != null)
                _buildContactRow(link_64!, contactData?.socialMediaUrl2),
