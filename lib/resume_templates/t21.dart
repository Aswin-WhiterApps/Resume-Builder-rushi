...
class Template21 {
  ...
  Template21({required this.resume21themeModel}) {
    ...
    linkAsset = resume21themeModel.linkIc ?? PdfAssets.link7;
  ...
  Future<bool> initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      throw ArgumentError(
          "User or Resume ID is null in Template21. Ensure MySingleton.userId and MySingleton.resumeId are set.");
    }

    final fireUser = FireUser();

    try {
      print("Template21: Fetching all data for PDF generation...");
      ...
      if (resume != null) {
        ...
        // Use resume.title if needed, though T7 logic uses introData firstName/LastName mainly
      } else {
        throw StateError("Main resume document not found for Template21.");
      }
      ...
      if (bulletAsset != null && bulletAsset!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bulletAsset!).then((data) {
          ...
          bulletDarkImage = img;
          bulletLightImage =
              img; // Using same for both as we don't have separate in theme
        }));
      }

      if (backgroundPath != null && backgroundPath!.isNotEmpty) {
        imageFutures.add(rootBundle.load(backgroundPath!).then(
            (data) => back71Image = MemoryImage(data.buffer.asUint8List())));
      }
      ...
      if (introData?.imagePath != null && introData!.imagePath!.isNotEmpty) {
        ...
        if (await imageFile.exists()) {
          ...
          print(
              "Template21: Profile image file not found at ${introData!.imagePath}");
        }
      }
      ...
      throw Exception("Failed to initialize data/assets in Template21: $e");
    }
  }
  ...
  Future<File> createPage() async {
    ...
    try {
      ...
      throw Exception("Template21 initialization failed: $e");
    }
    ...
    pdf.addPage(MultiPage(
        pageTheme: PageTheme(
            ...
            buildBackground: (context) {
              return Container(
                  color: PdfColor(0.995, 0.96, 0.98, 1), // Base color from T7
                  child: back71Image != null
                      ? Image(back71Image!, fit: BoxFit.cover)
                      : null);
            }),
    ...
    try {
      ...
      final filePath = "${outputDir.path}/Resume_T21_$timestamp.pdf";
      final file = File(filePath);

      await file.writeAsBytes(await pdf.save());
      print("Template21: PDF saved to $filePath");
      return file;
    } catch (e) {
      throw Exception("Failed to save Template21 PDF: $e");
    }
  }

  // Copied and adapted from T7
        ...
        child: Column(
          ...
          children: [
            ...
            if (educationData.isNotEmpty) ...[
              _getHeaderTextDark(text: "EDUCATION", size: 11),
              SizedBox(height: 4),
            ...
            if (workData.isNotEmpty) ...[
              _getHeaderTextDark(text: "WORK", size: 11),
              SizedBox(height: 4),
  ...
  Widget _getContactText({required String text}) => Text(
        ...
        style: TextStyle(
          font: normalFont,
          color:
              normalTextColor, // Adjusted to use normalTextColor which is black
          fontSize: 7,
