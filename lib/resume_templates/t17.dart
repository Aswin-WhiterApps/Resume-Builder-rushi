...
class Template17 {
  ...
  MemoryImage? imageBorder61;
  MemoryImage? email_61;
  MemoryImage? phone_61;
  MemoryImage? link_61;
  ...
  Template17(
      {required Resume17ThemeModel? resume17theme}) {
    subHeaderColor = resume17theme?.subHeaderColor ?? PdfColor.fromHex("#B87397");
    headerColor = resume17theme?.headerColor ?? PdfColor.fromHex("#B87397");
    nameColor = resume17theme?.nameColor ?? PdfColor.fromHex("#B87397");
    normalTextColor = resume17theme!.normalTextColor ?? PdfColor.fromHex("#0D490E");
    bullet61 = resume17theme.bullet61;
    background = resume17theme.rect61 ?? PdfAssets.rect61;
    imageBorder = resume17theme.imageBorder61;
    emailIc = resume17theme.emailIc;
    phoneIc = resume17theme.phoneIc;
    linkIc = resume17theme.linkIc;
    normalFont = resume17theme.normalFont ?? Font.helvetica();
    headerFont = resume17theme.headerFont ?? Font.helveticaBold(); 
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      print("Error in Template17: User or Resume ID is null.");
      return false;
    }
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template17: Main resume document not found.");
      }
      ...
      if (imageBorder != null && imageBorder!.isNotEmpty) {
        imageFutures.add(rootBundle.load(imageBorder!).then(
            (data) => imageBorder61 = MemoryImage(data.buffer.asUint8List())));
      }

      if (bullet61 != null && bullet61!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bullet61!).then((byteData) {
      ...
      if (emailIc != null && emailIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(emailIc!)
            .then((data) => email_61 = MemoryImage(data.buffer.asUint8List())));
      }
      if (phoneIc != null && phoneIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(phoneIc!)
            .then((data) => phone_61 = MemoryImage(data.buffer.asUint8List())));
      }
      if (linkIc != null && linkIc!.isNotEmpty) {
        ...
        imageFutures.add(rootBundle
            .load(linkIc!)
            .then((data) => link_61 = MemoryImage(data.buffer.asUint8List())));
      }

      if (background != null && background!.isNotEmpty) {
        imageFutures.add(rootBundle.load(background!).then((data) =>
            imageBorder61 = MemoryImage(data.buffer.asUint8List())));
      }
      ...
      if (introData?.imagePath != null && introData!.imagePath!.isNotEmpty) {
        ...
        if (await imageFile.exists()) {
          imageFutures.add(imageFile
              .readAsBytes()
              .then((bytes) => imageBorder61 = MemoryImage(bytes)));
        }
      }

      await Future.wait(imageFutures);
      print("Template17: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template17: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template17: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template17: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template17: Building PDF widgets...");
    print(
        " Template17: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template17: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template17: summeryData=${summeryData != null ? 'exists' : 'null'}");
    print(" Template17: workData.length=${workData.length}");
    print(" Template17: educationData.length=${educationData.length}");
    print(" Template17: sectionData.length=${sectionData.length}");
    ...
    pdf.addPage(MultiPage(
        pageTheme: PageTheme(
            ...
            buildBackground: (context) {
              return imageBorder61 != null
                  ? Container(
                      decoration: BoxDecoration(
                          image: DecorationImage(
                              fit: BoxFit.cover, image: imageBorder61!)))
          ...
          children: [
            if (imageBorder != null)
              Container(
                ...
                height: 120,
                padding:
                    imageBorder61 != null ? EdgeInsets.all(5) : EdgeInsets.zero,
                decoration: imageBorder61 != null
                    ? BoxDecoration(
                        image: DecorationImage(
                            image: imageBorder61!, fit: BoxFit.cover),
                ...
                child: Container(
                  ...
                  child:
                      ClipOval(child: Image(imageBorder61!, fit: BoxFit.cover)),
                ),
              ),
          ],
        ),
            ...
            Padding(
              ...
              child: _buildRotatedSection(
                ...
                content: Column(
                  ...
                  children: [
                    if (emailIc != null && email_61 != null)
                      _buildContactRow(email_61!, contactData?.email),
                    if (phoneIc != null && phone_61 != null)
                      _buildContactRow(phone_61!, contactData?.phone),
                    if (linkIc != null && link_61 != null)
                      _buildContactRow(link_61!, contactData?.socialMediaUrl1),
                    if (linkIc != null && link_61 != null)
                      _buildContactRow(link_61!, contactData?.socialMediaUrl2),
