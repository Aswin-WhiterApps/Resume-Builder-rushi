...
class Template24 {
  ...
  late final String? bullet81;
  late final String? background;
  late final String? imageBorderColor;
  ...
  Template24({required Resume24ThemeModel? resume24themeModel}) {
    subHeaderColor =
        resume24themeModel?.subHeaderColor ?? PdfColor.fromHex("#B87397");
    headerColor =
        resume24themeModel?.headerColor ?? PdfColor.fromHex("#B87397");
    nameColor = resume24themeModel?.nameColor ?? PdfColor.fromHex("#B87397");
    normalTextColor =
        resume24themeModel!.normalTextColor ?? PdfColor.fromHex("#0D490E");
    backgroundColor = resume24themeModel.backgroundColor;
    bullet81 = resume24themeModel.bullet81;
    background = resume24themeModel.background;
    phoneIc = resume24themeModel.phoneIc;
    linkIc = resume24themeModel.linkIc;
    normalFont = resume24themeModel.normalFont ?? Font.helvetica();
    headerFont = resume24themeModel.headerFont ?? Font.helveticaBold();
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      print("Error in Template24: User or Resume ID is null.");
      return false;
    }
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template24: Main resume document not found.");
      }
      ...
      if (bullet81 != null && bullet81!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bullet81!).then((byteData) {
      ...
      print("Template24: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template24: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template24: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template24: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template24: Building PDF widgets...");
    print(
        " Template24: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template24: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template24: summeryData=${summeryData != null ? 'exists' : 'null'}");
    print(" Template24: workData.length=${workData.length}");
    print(" Template24: educationData.length=${educationData.length}");
    print(" Template24: sectionData.length=${sectionData.length}");
      ...
      Container(
        ...
        decoration: BoxDecoration(
          ...
          gradient: LinearGradient(
            colors: [
              PdfColor.fromHex('#DDC76A'),
              PdfColor.fromHex('#D4BC5A'),
            ],
