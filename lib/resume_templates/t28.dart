...
class Template28 {
  ...
  Resume28ThemeModel resume28themeModel;
  ...
  Template28({required this.resume28themeModel}) {
    normalTextColor = resume28themeModel.normalTextColor ?? PdfColors.black;
    headerDarkColor = resume28themeModel.headerColor ?? PdfColors.grey800;
    // headerLightColor is not in Resume21ThemeModel, using default or derived
    headerLightColor = PdfColors.white;

    nameColor = resume28themeModel.nameColor ?? PdfColors.blueGrey900;
    positionColor = resume28themeModel.positionColor ?? PdfColors.blueGrey900;

    backgroundPath = resume28themeModel.background ?? PdfAssets.back102;
    linkAsset = resume28themeModel.linkIc ?? PdfAssets.link7;

    normalFont = resume28themeModel.normalFont ?? Font.times();
    headerFont = resume28themeModel.headerFont ?? Font.helvetica();
    nameFont = resume28themeModel.headerFont ?? Font.helveticaBold();
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      print("Error in Template28: User or Resume ID is null.");
      return false;
    }
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template28: Main resume document not found.");
      }
      ...
      print("Template28: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template28: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template28: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template28: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template28: Building PDF widgets...");
    print(
        " Template28: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template28: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template28: summaryData=${summaryData != null ? 'exists' : 'null'}");
    print(" Template28: workData.length=${workData.length}");
    print(" Template28: educationData.length=${educationData.length}");
    print(" Template28: sectionData.length=${sectionData.length}");
