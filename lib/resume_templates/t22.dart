...
class Template22 {
  ...
  Template22({required this.resume22themeModel}) {
    normalTextColor = resume22themeModel.normalTextColor ?? PdfColors.black;
    headerDarkColor = resume22themeModel.headerColor ?? PdfColors.grey800;
    // headerLightColor is not in Resume21ThemeModel, using default or derived
    headerLightColor = PdfColors.white;

    nameColor = resume22themeModel.nameColor ?? PdfColors.blueGrey900;
    positionColor = resume22themeModel.positionColor ?? PdfColors.blueGrey900;

    backgroundPath = resume22themeModel.background ?? PdfAssets.back72;
    linkIcPath = resume22themeModel.linkIc ?? PdfAssets.link82;

    normalFont = resume22themeModel.normalFont ?? Font.times();
    headerFont = resume22themeModel.headerFont ?? Font.helvetica();
    nameFont = resume22themeModel.headerFont ?? Font.helveticaBold();
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      print("Error in Template22: User or Resume ID is null.");
      return false;
    }
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template22: Main resume document not found.");
      }
      ...
      if (backgroundPath != null && backgroundPath!.isNotEmpty) {
        imageFutures.add(rootBundle.load(backgroundPath!).then((byteData) {
          ...
          print("Template22: Failed to load asset $backgroundPath: $e");
        }));
      }

      if (linkIcPath != null && linkIcPath!.isNotEmpty) {
        imageFutures.add(rootBundle.load(linkIcPath!).then((data) {
          link8Image = MemoryImage(data.buffer.asUint8List());
        }).catchError((e) {
          print("Template22: Failed to load asset $linkIcPath: $e");
        }));
      }

      // Load bullet images
      bulletDarkPath = PdfAssets.bullet7;
      if (bulletDarkPath != null && bulletDarkPath!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bulletDarkPath!).then((data) {
          ...
          bulletLightImage =
              MemoryImage(data.buffer.asUint8List()); // Use same for light
        }).catchError((e) {
          print("Template22: Failed to load asset $bulletDarkPath: $e");
        }));
      }
      ...
      print("Template22: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template22: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template22: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template22: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template22: Building PDF widgets...");
    print(
        " Template22: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template22: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template22: summaryData=${summaryData != null ? 'exists' : 'null'}");
    print(" Template22: workData.length=${workData.length}");
    print(" Template22: educationData.length=${educationData.length}");
    print(" Template22: sectionData.length=${sectionData.length}");

    final pdf = Document();

    print(" Template22: All widgets built successfully");
    ...
    try {
      ...
      final file = File("${outputDir.path}/Resume_T22_$timestamp.pdf");
      await file.writeAsBytes(await pdf.save());
      print("Pdf Saved To ${file.path}");
      return file;
    } catch (e) {
      print('Failed to save PDF for Template22: $e');
      return null;
    }
  }
        ...
        child: _buildRotatedSection(
          ...
          content: Column(
            ...
            children: [
              ...
              if (link8Image != null)
                _buildContactRow(link8Image!, contactData?.socialMediaUrl2),
            ],
          ),
        ),
      ),
