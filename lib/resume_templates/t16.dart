...
class Template16 {
  ...
  MemoryImage? imageBorder54;
  MemoryImage? email_54;
  MemoryImage? phone_54;
  MemoryImage? link_54;
  ...
  Template16({required Resume16ThemeModel? resume16theme}) {
    subHeaderColor =
        resume16theme?.subHeaderColor ?? PdfColor.fromHex("#1E3A8A");
    headerColor = resume16theme?.headerColor ?? PdfColor.fromHex("#1E3A8A");
    nameColor = resume16theme?.nameColor ?? PdfColor.fromHex("#1E3A8A");
    normalTextColor =
        resume16theme?.normalTextColor ?? PdfColor.fromHex("#000000");
    bullet54 = resume16theme?.bullet54;
    background = resume16theme?.back54 ?? PdfAssets.back54;
    imageBorder = resume16theme?.imageBorder54 ?? PdfAssets.imageBorder54;
    emailIc = resume16theme?.emailIc ?? PdfAssets.email54;
    phoneIc = resume16theme?.phoneIc ?? PdfAssets.phone54;
    linkIc = resume16theme?.linkIc ?? PdfAssets.link54;
    normalFont = resume16theme?.normalFont ?? Font.helvetica();
    headerFont = resume16theme?.headerFont ?? Font.helveticaBold();
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    if (userId == null || resumeId == null) {
      print("Error in Template16: User or Resume ID is null.");
      return false;
    }
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template16: Main resume document not found.");
      }
      ...
      if (imageBorder != null && imageBorder!.isNotEmpty) {
        imageFutures.add(rootBundle.load(imageBorder!).then(
            (data) => imageBorder54 = MemoryImage(data.buffer.asUint8List())));
      }

      if (bullet54 != null && bullet54!.isNotEmpty) {
        imageFutures.add(rootBundle.load(bullet54!).then((byteData) {
      ...
      if (emailIc != null && emailIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(emailIc!)
            .then((data) => email_54 = MemoryImage(data.buffer.asUint8List())));
      }
      if (phoneIc != null && phoneIc!.isNotEmpty) {
        imageFutures.add(rootBundle
            .load(phoneIc!)
            .then((data) => phone_54 = MemoryImage(data.buffer.asUint8List())));
      }
      if (linkIc != null && linkIc!.isNotEmpty) {
        ...
        imageFutures.add(rootBundle
            .load(linkIc!)
            .then((data) => link_54 = MemoryImage(data.buffer.asUint8List())));
      }
      ...
      print("Template16: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template16: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template16: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template16: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template16: Building PDF widgets...");
    print(
        " Template16: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template16: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template16: summeryData=${summeryData != null ? 'exists' : 'null'}");
    print(" Template16: workData.length=${workData.length}");
    print(" Template16: educationData.length=${educationData.length}");
    print(" Template16: sectionData.length=${sectionData.length}");
          ...
          children: [
            if (imageBorder != null)
              Container(
                ...
                height: 120,
                padding:
                    imageBorder54 != null ? EdgeInsets.all(5) : EdgeInsets.zero,
                decoration: imageBorder54 != null
                    ? BoxDecoration(
                        image: DecorationImage(
                            image: imageBorder54!, fit: BoxFit.cover),
                ...
                child: Container(
                  ...
                  child:
                      ClipOval(child: Image(imageBorder54!, fit: BoxFit.cover)),
                ),
              ),
          ],
        ),
            ...
            Padding(
              ...
              child: _buildRotatedSection(
                ...
                content: Column(
                  ...
                  children: [
                    if (emailIc != null && email_54 != null)
                      _buildContactRow(email_54!, contactData?.email),
                    if (phoneIc != null && phone_54 != null)
                      _buildContactRow(phone_54!, contactData?.phone),
                    if (linkIc != null && link_54 != null)
                      _buildContactRow(link_54!, contactData?.socialMediaUrl1),
                    if (linkIc != null && link_54 != null)
                      _buildContactRow(link_54!, contactData?.socialMediaUrl2),
