...
class Template29 {
  ...
  Resume29ThemeModel resume29themeModel;
  ...
  Template29({required this.resume29themeModel}) {
    normalTextColor = resume29themeModel.normalTextColor ?? PdfColors.black;
    headerDarkColor = resume29themeModel.headerColor ?? PdfColors.grey800;
    // headerLightColor is not in Resume21ThemeModel, using default or derived
    headerLightColor = PdfColors.white;

    nameColor = resume29themeModel.nameColor ?? PdfColors.blueGrey900;
    positionColor = resume29themeModel.positionColor ?? PdfColors.blueGrey900;

    backgroundPath = resume29themeModel.background ?? PdfAssets.back103;
    linkAsset = resume29themeModel.linkIc ?? PdfAssets.link7;

    normalFont = resume29themeModel.normalFont ?? Font.times();
    headerFont = resume29themeModel.headerFont ?? Font.helvetica();
    nameFont = resume29themeModel.headerFont ?? Font.helveticaBold();
  }
  ...
  Future<bool> _initializeDataAndAssets() async {
    ...
    try {
      ...
      if (resume != null) {
        ...
        print("Template29: Main resume document not found.");
      }
      ...
      print("Template29: All assets loaded successfully.");
      return true;
    } catch (e) {
      print("Error initializing data/assets in Template29: $e");
      return false;
    }
  }

  Future<File?> createPage() async {
    print(" Template29: Starting PDF generation...");
    final bool isInitialized = await _initializeDataAndAssets();
    if (!isInitialized) {
      print(" Template29: PDF generation failed due to initialization errors.");
      return null;
    }

    print(" Template29: Building PDF widgets...");
    print(
        " Template29: introData=${introData != null ? '${introData?.firstName ?? ''} ${introData?.lastName ?? ''}' : 'null'}");
    print(
        " Template29: contactData=${contactData != null ? 'exists' : 'null'}");
    print(
        " Template29: summaryData=${summaryData != null ? 'exists' : 'null'}");
    print(" Template29: workData.length=${workData.length}");
    print(" Template29: educationData.length=${educationData.length}");
    print(" Template29: sectionData.length=${sectionData.length}");
