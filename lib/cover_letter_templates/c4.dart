...
class CoverLetter4{
  ...
  late String back;
  late String borderImage;
  // late MemoryImage phoneIc;
  CL4ThemeModel cl4themeModel;
  ResumeModel? resumeData;
  
  CoverLetter4({required this.cl4themeModel, this.resumeData}){
    normalFont  =cl4themeModel.normalFont!;
    nameFont  =cl4themeModel.nameFont!;

    normalTextColor  =cl4themeModel.normalTextColor!;
    imageBorder  =cl4themeModel.imageBorder!;
    nameColor  =cl4themeModel.nameColor!;
    back  =cl4themeModel.background!;
    borderImage  =cl4themeModel.borderImage!;
  }
  ...
  Future<void> getFonts() async {
    try {
      background = MemoryImage((await rootBundle
          ...
          .buffer
          .asUint8List());
      borderImg = MemoryImage((await rootBundle
          .load(borderImage))
          .buffer
          .asUint8List());
  ...
  Widget coverLetter4() {
    // PdfColor verticalObjColor =PdfColor.fromHex("#B27B80");

    // PdfColor backgroundColor =  PdfColor.fromHex("#FAF5F5");
    // PdfColor headerLight =PdfColor.fromHex("#F8EDED");
